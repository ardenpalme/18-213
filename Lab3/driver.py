...
def test_trans():
    ...
    if transOK:
        ...
        # 1024x1024 transpose
        print("Running ./test-trans -s -M 1024 -N 1024")
        p = subprocess.Popen("./test-trans -s -M 1024 -N 1024 -l | grep TEST_TRANS_RESULTS",
                             shell=True, stdout=subprocess.PIPE, encoding='utf-8')
        stdout_data = p.communicate()[0]
        result1024 = re.findall(r'(\d+)', stdout_data)

        cycles32 = int(result32[1])
        cycles1024 = int(result1024[1])
...
def main():
    ...
    if autograde:
        autoresult = "{\"scores\": {\"Autograded\": %.1f}, \"scoreboard\": [%.1f, %d, %d, %d]}" % (
            total_score, total_score, cycles32+cycles1024, cycles32, cycles1024)
        print(autoresult)

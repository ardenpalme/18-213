...
static void trans_tmp(size_t M, size_t N, const double A[N][M], double B[M][N],
    ...
    for (size_t i = 0; i < N; i++) {
        for (size_t j = 0; j < M; j++) {
            size_t di = i % 2;
            size_t dj = j % 2;
            tmp[i % TMPCOUNT] = A[i][j];
            B[j][i] = tmp[i % TMPCOUNT];
            tmp[2 * di + dj] = A[i][j];
            B[j][i] = tmp[2 * di + dj];
        }
    }
...
static void transpose_submit(size_t M, size_t N, const double A[N][M],
                             double B[M][N], double tmp[TMPCOUNT]) {
    if (M == 32 && N == 32) {
        // iterate through all 16 (8x8) blocks
        for (size_t ii = 0; ii < (M / 8); ii++) {
            for (size_t jj = 0; jj < (M / 8); jj++) {

                // for each 4x4 block transpose w/o tmp
                for (size_t i = (M / 4) * ii; i < (M / 4) * (ii + 1); i++) {
                    for (size_t j = (M / 4) * jj; j < (M / 4) * (jj + 1); j++) {
                        B[j][i] = A[i][j];
                    }
                }
            }
    ...
    if (M == 1024 && N == 1024) {
        for (size_t ii = 0; ii < (M / 8); ii++) {
            for (size_t jj = 0; jj < (M / 8); jj++) {
                ...
                // for each 4x4 block transpose w/o tmp
                for (size_t i = (M / 128) * ii; i < (M / 128) * (ii + 1); i++) {
                    for (size_t j = (M / 128) * jj; j < (M / 128) * (jj + 1);
                         j++) {
                        B[j][i] = A[i][j];
                    }
                }
 ...
 *
 * At runtime, the driver will evaluate each function registered here,
 * and and summarize the performance of each. This is a handy way to
 * experiment with different transpose strategies.
 */
